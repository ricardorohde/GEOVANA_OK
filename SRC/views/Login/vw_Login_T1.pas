...
procedure Tfrm_Login_T1.bSairClick(Sender: TObject);
begin
   modalresult := mrcancel;
   close;
end;

function Tfrm_Login_T1.DadosCorretos:Boolean;
begin
    result := false;
...
procedure Tfrm_Login_T1.Tratar_Validade_de_Certificado_Digital;
begin
...
procedure Tfrm_Login_T1.Encerrar_a_Aplicacao;
begin
   Destroi_Objetos_das_Classes;
   close;
end;

procedure Tfrm_Login_T1.AbreMenuPrincipal;
begin
...
procedure Tfrm_Login_T1.edEmpresaExit(Sender: TObject);
begin
...
procedure Tfrm_Login_T1.edEmpresaKeyPress(Sender: TObject; var Key: Char);
begin
     if key = #13 Then
     begin
        key := #0;
...
procedure Tfrm_Login_T1.edt_senhaExit(Sender: TObject);
begin
  if bSair.Focused         then exit;
  if bAcessoRemoto.Focused then exit;
...
procedure Tfrm_Login_T1.edt_senhaKeyPress(Sender: TObject; var Key: Char);
begin
     if key = #13 Then
     begin
        key := #0;
...
procedure Tfrm_Login_T1.edt_usuarioExit(Sender: TObject);
begin
  //if edEmpresa.focused     then exit;
  //if bSair.Focused         then exit;
  //if bAcessoRemoto.Focused then exit;
...
procedure Tfrm_Login_T1.edt_usuarioKeyPress(Sender: TObject; var Key: Char);
begin
     if key = #13 Then
     begin
        key := #0;
...
procedure Tfrm_Login_T1.Preencher_Dados_do_Ultimo_Acesso;
...
procedure Tfrm_Login_T1.Desabilitar_Campos_e_Botão_Entrar;
begin
